...
def load_unit_map() -> Dict[str, str]:
    # Default mapped non-rt services are controller-only.
    # Other nodes should only publish their discovered rt-*.service units
    # unless explicitly given RT_UNIT_MAP_JSON.
                ...
                try:
                    ...
                    for key in r.scan_iter(match=f"{SERVICE_PREFIX}*"):
                        ...
                        if sid not in keep:
                            owner = ""
                            try:
                                owner = str(r.hget(key, "ownerNode") or "").strip()
                            except Exception:
                                owner = ""

                            # In distributed mode, only prune keys this node owns.
