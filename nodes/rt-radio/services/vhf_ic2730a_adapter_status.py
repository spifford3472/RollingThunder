...
def process_request_if_needed(
    ...
    elif action == "manual_select_fast_tune_repeater_vfo":
        result = adapter.manual_select_fast_tune_repeater_vfo(
            request.get("repeater") if isinstance(request.get("repeater"), dict) else {},
            force_full_tune=boolish(request.get("force_full_tune"), False),
        )
        result["request_id"] = request_id
        result["action"] = action
        result["reason_request"] = str(request.get("reason") or "")
        result["selected_id"] = str(request.get("selected_id") or "")
        result["selected_index"] = request.get("selected_index")
        result["source"] = SOURCE
        result["updated_utc"] = utc_now_iso()
